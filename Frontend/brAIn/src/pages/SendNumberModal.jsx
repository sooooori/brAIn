...
const SendNumberModal = ({ isOpen, onRequestClose }) => {
    ...
    const handleSendCode = () => {
        ...
        axios.post(`${import.meta.env.VITE_API_BASE_URL}/v1/members/checkEmail`, { email : email })
            .then(response => {
                console.log(response)
                if (response.data.message === 'Email check successfully') {
                    // If email exists, send the verification code
                    return axios.post(`${import.meta.env.VITE_API_BASE_URL}/v1/members/sendAuthNumber`, { email : email });
                } else {
